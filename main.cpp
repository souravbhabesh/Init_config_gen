...
int main(int argc, char **argv)
{
  ...
  printf ("Thermalized position of right end %.8f\n",slider_thermal);
  ...
  for(int run=1;run<=RUN;run++)
  {
	...
	sprintf(thermalpos_file,"../Sim_dump_thermal/L%d/W%d/k%.1f/r%d/thermalPosFrame.bin",NX,NY,KAPPA,run+RUN*k);
	...
	sprintf(lattice_file,"../Sim_dump_thermal/L%d/W%d/k%.1f/r%d/lattice_thermal.dat",NX,NY,KAPPA,run+RUN*k);
